...
<div id="meet_list_content_staff" class="">
<?php
  foreach (get_communication_records($memberId) as $key => $value) {
    $date = yyyymmdd_to_ddmm($value['date']);
    echo "<div class='cursor-pointer str_record_staff' data-id='{$value['id']}'>{$date} — {$value['time']} Служащий: {$serving_ones_list[$value['serving_one']]}</div>";
  }
?>
</div>
