<?php
  ...
  class InputsGroup
  {
    static function radio ($list, $seleted_option_default = 'missing', $seleted_option_value = '')
    {
      foreach ($list as $key => $value) {
        ...
        /*if (!empty($seleted_option_default) && $value === $seleted_option_default) {
          $default = 'checked';
        }*/
        echo "<div class='form-check-inline'><label class='form-check-label'>
        <input type='radio' class='form-check-input' id='radio_point{$key}' name='radio_point' value='{$value}' {$default}>{$value}</label></div>";
      }
    }
