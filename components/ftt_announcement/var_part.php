<?php
  ...
  if (isset($_COOKIE['tab_active']) && $_COOKIE['tab_active'] === 'inbox') {
    ...
    $tab_two_active = '';
    $tab_three_active = '';
  } elseif (isset($_COOKIE['tab_active']) && $_COOKIE['tab_active'] === 'infoboard') {
    $tab_one_active = '';
    $tab_two_active = '';
    $tab_three_active = 'active';
  }
  include_once 'db/classes/FttExtraLists.php';
  if ($tab_three_active === 'active') {
    // БД
    require 'db/ftt/ftt_infoboard_db.php';
    // Extra data
    $types = ['schedule' => 'Расписание','announcements'=>'Объявления', 'hwmr'=>'ССУО', 'ls'=>'Жизнеизучение', 'verse'=>'Цитата'];
    $recipients_group = [];
    $list_trainee_full = [];
  } else {
    // БД
