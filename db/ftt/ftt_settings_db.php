...
// список таблиц
$tables = array('ftt_trainee', 'ftt_session', 'ftt_session_correction', 'ftt_announcement', 'ftt_announcement_recipients', 'ftt_attendance', 	'ftt_attendance_sheet', 'ftt_permission', 'ftt_permission_sheet', 'ftt_skip', 'ftt_fellowship', 'ftt_fellowship_tmpl', 'ftt_service', 'ftt_gospel', 'ftt_gospel_members', 'ftt_gospel_goals', 'ftt_extra_help', 'ftt_late', 'ftt_bible_new', 'ftt_prophecy');
...
function resetBible($adminId) {
  ...
  $exist = [];
  $res = db_query ("SELECT DISTINCT `member_key` FROM `ftt_bible_new`");
  while ($row = $res->fetch_assoc()) $exist[]=$row['member_key'];

  foreach ($exist as $value) {
    if (array_search($value, $trainee) === false) {
        $result = db_query ("DELETE FROM `ftt_bible_new` WHERE `member_key` = '{$value}'");
    }
  }

  write_to_log::warning(db_getMemberIdBySessionId (session_id()), "Данные таблицы ПВОМ ftt_bible_new и файлы частично удалены служащим с ключом {$adminId}. Результат: {$result}");
