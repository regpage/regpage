            ...
            for(var i in events){
              ...
              if (i === "0") {
                ...
                desctopEvent = '<div '+eventAttrs+'>'+
                            ...
                            +'</div><div style="padding-top:5px; padding-bottom: 30px; padding-left: 10px; border-bottom: 1px solid #DDD;" onmouseover="this.style.backgroundColor='+ "'" + '#fff' + "'" + '">'
                            + ((event.close_registration == 1 && regstateText !== 'регистрация подтверждена' && regstateText !== 'ожидание подтверждения') ? '<span style="color: red; margin-left: 0px; margin-top: -10px;">Регистрация закрыта ' + formatDDMM(event.end_date) +'</span>' : '<a class="btn btn-secondary event-row_btn" type="button" title="Отправить данные команде регистрации" style="margin-right: 10px;">Информация</a>'
...
console.log('stop is ', stopRegistration, 'close is ', closeRegistration, modalWindow);
                ...
                $('.list-events .event-row_btn').click(function(e){ // .list-events .event-row,
                    ...
                    $('.theActiveEvent').removeClass('theActiveEvent');
                    let event_row;
                    if ($(this).hasClass("event-row_btn")) {
                      event_row = $(this).parent().prev();
                    } else {
                      event_row = $(this);
                    }

                    event_row.addClass('theActiveEvent');
                    var eventId = event_row.attr('data-id');
                    arriveDepartMyself(event_row.attr('data-start_date'),'.emArrDate');
                    arriveDepartMyself(event_row.attr('data-end_date'),'.emDepDate');
